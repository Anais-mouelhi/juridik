...
export default function TachesPage() {
  ...
  const [form, setForm] = useState({ title: "", dossier_id: "", due_date: "", priority: "normal" });
  ...
  const handleCreate = async () => {
    ...
    await base44.entities.Task.create({ ...form, dossier_name: dossier?.title || '', completed: false });
    setForm({ title: "", dossier_id: "", due_date: "", priority: "normal" });
    setShowForm(false);
    ...
    <div className="p-6 lg:p-10 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-serif text-3xl font-semibold text-foreground">Tâches</h1>
        ...
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="space-y-3 mb-4">
            <input placeholder="Titre de la tâche *" value={form.title} onChange={e => setForm(f => ({...f, title: e.target.value}))}
              className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-foreground/30 transition-colors" />
            ...
            <select value={form.priority} onChange={e => setForm(f => ({...f, priority: e.target.value}))}
              className="bg-background border border-border rounded-xl px-4 py-2.5 text-sm text-foreground focus:outline-none focus:border-foreground/30 transition-colors w-full">
      ...
      {/* Pending grouped */}
      {pending.length === 0 ? (
        <div className="bg-card rounded-2xl border border-border flex flex-col items-center justify-center py-16 mb-6 card-shadow">
          <div className="h-12 w-12 rounded-2xl bg-muted flex items-center justify-center mb-3">
            <CheckSquare className="h-6 w-6 text-muted-foreground/30" />
          </div>
          <p className="text-foreground font-semibold">Aucune tâche en cours</p>
        ...
        <div className="space-y-5 mb-6">
          {Object.entries(grouped).map(([dossierName, groupTasks]) => (
            <div key={dossierName} className="bg-card rounded-2xl border border-border overflow-hidden card-shadow">
              <div className="px-5 py-3 border-b border-border bg-muted/30">
                <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{dossierName}</p>
              </div>
              <div className="divide-y divide-border">
                {groupTasks.map((task, i) => (
                  <motion.div key={task.id}
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                    className="flex items-center gap-3 px-5 py-3.5 group hover:bg-muted/20 transition-colors"
                  >
                    <button onClick={() => toggleComplete(task)} className="flex-shrink-0">
                      <Square className="h-4 w-4 text-muted-foreground/50 hover:text-foreground transition-colors" />
                    </button>
                    {task.priority === 'urgent' && <AlertCircle className="h-3.5 w-3.5 text-red-500 flex-shrink-0" />}
      ...
      {done.length > 0 && (
        <div className="bg-card rounded-2xl border border-border overflow-hidden card-shadow opacity-55">
          <div className="px-5 py-3 border-b border-border bg-muted/30">
            <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Terminées ({done.length})</p>
          </div>
          <div className="divide-y divide-border">
            {done.map(task => (
              <div key={task.id} className="flex items-center gap-3 px-5 py-3.5 group">
                <button onClick={() => toggleComplete(task)} className="flex-shrink-0">
                  <CheckSquare className="h-4 w-4 text-foreground/40" />
                </button>
